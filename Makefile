...
ifeq ($(DEBIAN_VERSION),9)
	...
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install debian-archive-keyring
	-apt-get -y $(APT_GET_FORCE) update
	@mkdir -p $(DIR_CFG)
	touch $(KEYRING)
...
$(INSTALL_CERTIFICATES):
	...
	make $(KEYRING)
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install ca-certificates
	-apt-get -y $(APT_GET_FORCE) update
	touch $(INSTALL_CERTIFICATES)

/usr/lib/$(LIB_ARCH)/libfftw3f.a:
	apt-get --no-install-recommends -y $(APT_GET_FORCE) install libfftw3-dev
...
/usr/bin/clang-6.0:
	...
	-apt-get -y $(APT_GET_FORCE) update
	apt-get --no-install-recommends -y $(APT_GET_FORCE) install clang-6.0

/usr/bin/clang-7:
	-apt-get -y $(APT_GET_FORCE) update
	apt-get --no-install-recommends -y $(APT_GET_FORCE) install clang-7

/usr/bin/clang-8:
	-apt-get -y $(APT_GET_FORCE) update
	apt-get --no-install-recommends -y $(APT_GET_FORCE) install clang-8

/usr/bin/clang-11:
	-apt-get -y $(APT_GET_FORCE) update
	apt-get --no-install-recommends -y $(APT_GET_FORCE) install clang-11

/usr/bin/curl:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install curl

/usr/bin/wget:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install wget

/usr/bin/htop:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install htop

/usr/sbin/avahi-autoipd:
	#-apt-get --no-install-recommends -y $(APT_GET_FORCE) install avahi-daemon avahi-utils libnss-mdns avahi-autoipd

/usr/bin/upnpc:
	#-apt-get --no-install-recommends -y $(APT_GET_FORCE) install miniupnpc

/usr/bin/dig:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install dnsutils

/usr/bin/pgmtoppm:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install netpbm

/sbin/ethtool:
	#-apt-get --no-install-recommends -y $(APT_GET_FORCE) install ethtool

/usr/bin/sshpass:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install sshpass

/usr/bin/killall:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install psmisc

/usr/bin/dtc:
	#-apt-get --no-install-recommends -y $(APT_GET_FORCE) install device-tree-compiler

ifeq ($(DEBIAN_10_AND_LATER),true)
/usr/include/openssl/ssl.h:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install openssl libssl1.1 libssl-dev

/usr/bin/connmanctl:
	#-apt-get --no-install-recommends -y $(APT_GET_FORCE) install connman
endif

ifeq ($(BBAI_64),true)
/usr/bin/cpufreq-info:
	#-apt-get --no-install-recommends -y $(APT_GET_FORCE) install cpufrequtils
endif
...
/usr/bin/jq:
	-apt-get --no-install-recommends -y $(APT_GET_FORCE) install jq
endif
...
/usr/bin/xz: $(INSTALL_CERTIFICATES)
	#apt-get --no-install-recommends -y $(APT_GET_FORCE) install xz-utils
