...
/* web/extensions/ext.min.js (web/extensions/ext.js = Fri Sep 16 09:51:42 2022) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Sep 14 10:12:01 2022) */
...
var ansi={colors:[[0,0,0],[194,54,33],[37,188,36],[173,173,39],[73,46,225],[211,56,211],[51,187,200],[203,204,205],[129,131,131],[252,57,31],[49,231,34],[234,236,35],[88,51,255],[249,53,248],[20,240,240],[233,235,235]],BRIGHT:8,RED:"\u001b[97m\u001b[101m",YELLOW:"\u001b[103m",GREEN:"\u001b[102m",CYAN:"\u001b[106m",BLUE:"\u001b[97m\u001b[104m",MAGENTA:"\u001b[97m\u001b[105m",GREY:"\u001b[47m",NORM:"\u001b[m",rolling:['RED','YELLOW','GREEN','CYAN','BLUE','MAGENTA','GREY'],rolling_n:7};function kiwi_output_sgr(p){var result='SGR',snew='';var sgr,saw_reset=0,saw_color=0;var sa=p.esc.s.substr(1).split(';');var sl=sa.length;if(p.isAltBuf)result+='('+p.r+','+p.c+')';for(var ai=0;ai<sl&&!isNumber(result);ai++){sgr=(sa[ai]==''||sa[ai]=='m')?0:parseInt(sa[ai]);if(sgr==0){p.sgr.fg=p.sgr.bg=null;result+=', reset';saw_reset=1;}else
...
if(sgr==7){var tf=p.sgr.fg;p.sgr.fg=p.sgr.bg;p.sgr.bg=tf;if(p.sgr.fg==null)p.sgr.fg=[255,255,255];if(p.sgr.bg==null)p.sgr.bg=[0,0,0];result+=', reverse video';saw_color=1;}else
if(sgr==27){p.sgr.fg=p.sgr.bg=null;result+=', reverse video off';saw_color=1;}else
if(sgr>=30&&sgr<=37){p.sgr.fg=ansi.colors[sgr-30+p.sgr.bright];result+=', fg color';saw_color=1;}else
if(sgr>=90&&sgr<=97){p.sgr.fg=ansi.colors[sgr-90+ansi.BRIGHT];result+=', fg color bright';saw_color=1;}else
if(sgr==39){p.sgr.fg=null;result+=', fg normal';saw_color=1;}else
if(sgr>=40&&sgr<=47){p.sgr.bg=ansi.colors[sgr-40+p.sgr.bright];result+=', bg color';saw_color=1;}else
if(sgr>=100&&sgr<=107){p.sgr.bg=ansi.colors[sgr-100+ansi.BRIGHT];result+=', bg color bright';saw_color=1;}else
if(sgr==49){p.sgr.bg=null;result+=', bg normal';saw_color=1;}else
if(sgr==38||sgr==48){var n8,r,g,b,color,ci;if(sl==3&&(parseInt(sa[1])==5)&&(!isNaN(n8=parseInt(sa[2])))){ai+=2;if(n8<=15){color=ansi.colors[n8];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;result+=', 38/48 mode color';saw_color=1;}else
if(n8<=231){n8-=16;r=Math.floor(n8/36);n8-=r*36;g=Math.floor(n8/6);n8-=g*6;b=n8;r=Math.floor(255*r/5);g=Math.floor(255*g/5);b=Math.floor(255*b/5);color=[r,g,b];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;result+=', color cube';saw_color=1;}else
if(n8<=255){ci=8+(n8-232)*10;color=[ci,ci,ci];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;result+=', grayscale ramp';saw_color=1;}else
result=2;}else
if(sl==5&&(parseInt(sa[1])==2)&&(!isNaN(r=parseInt(sa[2])))&&(!isNaN(g=parseInt(sa[3])))&&(!isNaN(b=parseInt(sa[4])))){r=w3_clamp(r,0,255);g=w3_clamp(g,0,255);b=w3_clamp(b,0,255);color=[r,g,b];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;result+=', 24-bit color';saw_color=1;}else
result=2;}else
result=2;}
if(saw_reset){if(p.sgr.span){snew+='</span>';p.sgr.span=0;}}else
if(saw_color){if(p.sgr.span)snew+='</span>';snew+='<span style="'+(p.sgr.fg?('color:'+kiwi_rgb(p.sgr.fg)+';'):'')+(p.sgr.bg?('background-color:'+kiwi_rgb(p.sgr.bg)+';'):'')+'">';p.sgr.span=1;}else{}
return{result:result,snew:snew};}
var pp;function kiwi_output_msg(id,id_scroll,p){var i,j;var dbg=(1&&dbgUs);if(dbg)pp=p;var parent_el=w3_el(id);if(!parent_el){console.log('kiwi_output_msg NOT_FOUND id='+id);return;}
var appendEmptyLine=function(parent_el){return w3_create_appendElement(parent_el,'pre','');};var removeAllLines=function(parent_el){while(parent_el.firstChild){parent_el.removeChild(parent_el.firstChild);}};var render=function(){var fg=null,bg=null,span=false;for(var r=1;r<=p.rows;r++){p.dirty[r]=false;var s='';for(var c=1;c<=p.cols;c++){var color=p.color[r][c];if(color.fg!=fg||color.bg!=bg){if(span)s+='</span>';if(color.fg||color.bg){s+='<span style="'+(color.fg?('color:'+kiwi_rgb(color.fg)+';'):'')+
(color.bg?('background:'+kiwi_rgb(color.bg)+';'):'')+'">';}else{span=false;}
fg=color.fg;bg=color.bg;span=true;}
var ch=p.screen[r][c];if(1&&p.show_cursor&&r==p.r&&c==p.c){console.log('cursor '+p.r+','+p.c);s+='<span style="color: white; background: black;">'+ch+'</span>';}else{s+=ch;}}
if(s=='')s='&nbsp;';else
if(span)s+='</span>';try{p.els[r].innerHTML=s;}catch(ex){console.log('r='+r);console.log(p.els[r]);console.log(ex);}}};var sched=function(){kiwi_clearTimeout(p.rend_timeout);p.rend_timeout=setTimeout(function(){render();},250);};var dirty=function(){p.dirty[p.r]=true;};var screen_char=function(ch){var r=p.r,c=p.c;if(p.insertMode){for(i=r.cols-1;i>=c;i--){p.screen[r][i+1]=p.screen[r][i];p.color[r][i+1]=p.color[r][i];}}
p.screen[r][c]=ch;p.color[r][c]={fg:p.sgr.fg,bg:p.sgr.bg};dirty();p.c++;if(p.c>p.cols){p.c=1;p.r++;if(p.r>p.rows){p.r=1;dirty();}}
sched();};var s;try{s=kiwi_decodeURIComponent('kiwi_output_msg',p.s);}catch(ex){console.log('decodeURIComponent FAIL:');console.log(p.s);s=p.s;}
if(!p.init){removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.esc={s:'',state:0};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.inc=1;p.r=p.c=1;p.show_cursor=false;p.insertMode=true;p.screen=[];p.color=[];p.dirty=[];p.els=[];for(var r=0;r<=p.rows;r++){p.screen[r]=[];p.color[r]=[];}
p.isAltBuf=false;p.alt_save='';p.traceEvery=false;p.rend_timeout=null;p.init=true;}
var snew='';var el_scroll=w3_el(id_scroll);var wasScrolledDown=null;if(isUndefined(p.tstr))p.tstr='';if(isUndefined(p.col))p.col=0;if(p.process_return_alone&&s.charAt(0)=='\r'&&(s.length==1||s.charAt(1)!='\n')){if(p.isAltBuf){}else{s=s.substring(1);p.tstr=snew='';p.col=0;}}
if(0){if(dbg)console.log('kiwi_output_msg:');if(dbg)console.log(JSON.stringify(p));if(dbg)console.log(JSON.stringify(s));}
if(p.remove_returns)s=s.replace(/\r/g,'');if(p.inline_returns&&!p.isAltBuf){s=s.replace(/\r\n/g,'\n');s=s.replace(/\r\n/g,'\n');}
if(dbg)console.log(JSON.stringify(s));for(var si=0;si<s.length;si++){var result=null;var c=s.charAt(si);if(p.inline_returns&&p.return_pending&&c!='\r'){if(p.isAltBuf){}else{p.tstr=snew='';p.col=0;p.return_pending=false;}}
if(c=='\f'){if(p.isAltBuf){}else{removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.tstr=snew='';p.col=0;}}else
if(c=='\r'){if(p.isAltBuf){p.c=1;result='\\r (col = 1)';}
if(p.inline_returns){p.return_pending=true;}}else
if(c=='\b'){if(p.isAltBuf){if(p.c>1){dirty();p.c--;}
result='backspace (arrow left)';}else{if(p.tstr.length>1)
p.tstr=p.tstr.slice(0,p.tstr.length-1);}}else
if(c=='\t'){if(p.isAltBuf){}else{snew+='&nbsp;';p.col++;while((p.col&7)!=0){snew+='&nbsp;';p.col++;}}}else
if(c=='\x1b'){p.esc.s='';p.esc.state=1;p.traceEvery=false;}else
if(p.esc.state==1){if((c<'@'&&c!='>')||c=='['){p.esc.s+=c;}else{p.esc.s+=c;var first=p.esc.s.charAt(0);var second=p.esc.s.charAt(1);var last_hl=(c=='h'||c=='l');var enable=(c=='h');result=0;var n1=1,n2=1;var t=p.esc.s.slice(1);if(t.length>1&&isNumber(+t[0])){var a=t.slice(0,t.length-1);a=a.split(';');n1=+a[0];n1=n1||1;if(a.length>1)n2=+a[1];n2=n2||1;}
if(first=='['){if(c=='m'){var rv=kiwi_output_sgr(p);result=rv.result;if(p.isAltBuf){}else{snew+=rv.snew;}
p.traceEvery=true;}else
if(c=='J'&&p.isAltBuf){var r_start,r_end,c_start,c_end;if(second=='0'||second=='J'){r_start=p.r,r_end=p.rows;c_start=p.c,c_end=p.cols;result='erase cur to EOS';}else
...
for(var r=r_start;r_start&&r<=r_end;r++){for(var c=c_start;c<=((r==p.r)?c_end:p.cols);c++){p.screen[r][c]=' ';p.color[r][c]={fg:null,bg:null};}
p.dirty[r]=true;}
sched();}else
if(c=='K'){if(p.isAltBuf){var start,end;result='erase in line: ';switch(second){case'0':case'K':start=p.c;end=p.cols;result+='cur to EOL';break;case'1':start=1;end=p.c;result+='BOL to cur';break;case'2':start=1;end=p.cols;result+='full line';break;default:start=0;break;}
for(var c=start;start&&c<=end;c++){p.screen[p.r][c]=' ';p.color[p.r][c]={fg:null,bg:null};}
dirty();sched();}}else
if(c=='H'){result='move '+n1+','+n2;dirty();p.r=n1;p.c=n2;dirty();}else
if(c=='d'){result='move row '+n1;dirty();p.r=n1;dirty();}else
if(c=='G'){result='move col'+n1;p.c=n1;}else
if(second=='?'&&last_hl){n1=parseInt(p.esc.s.substr(2));result=(enable?'SET':'RESET')+' ';switch(n1){case 1:result+='automatic margins';break;case 7:result+='vertical autowrap';break;case 12:result+='(part of RESET cursor visible)';break;case 25:result+='cursor visible';p.show_cursor=true;break;case 1049:result+='alt screen buf';if(dbg&&enable&&!p.isAltBuf){p.alt_save=parent_el.innerHTML;removeAllLines(parent_el);for(var r=1;r<=p.rows;r++){p.dirty[r]=false;try{for(var c=1;c<=p.cols;c++){p.screen[r][c]=' ';p.color[r][c]={fg:null,bg:null};}}catch(ex){console.log('--------');console.log('r='+r+' c='+c);console.log(p.screen);console.log(ex);}
p.els[r]=appendEmptyLine(parent_el);p.els[r].innerHTML='&nbsp;';}
p.isAltBuf=true;}else
if(!enable&&p.isAltBuf){console.log('$EXIT alt buf');console.log(kiwi_JSON(p.alt_save));parent_el.innerHTML=p.alt_save;p.el=appendEmptyLine(parent_el);p.isAltBuf=false;}
break;default:result+='$UNKNOWN ='+n1;break;}}else
if(c=='r'){result='set margins, top='+n1+', bottom='+n2;}else
...
dirty();sched();result='eraseX #'+n1+' chars';}else
if(c=='P'&&p.isAltBuf){if(n1==0)n1=1;var col;for(var ci=0,col=p.c;ci<n1&&col<=p.cols;ci++,col++){p.screen[p.r][col]=' ';p.color[p.r][col]={fg:null,bg:null};}
dirty();sched();result='del #'+n1+' chars';}else
if(c=='S'&&p.isAltBuf){result='pan down '+n1;}else
if(c=='A'){if(p.r>1){dirty();p.r--;dirty();}
result='arrow up';}else
if(c=='B'){if(p.r<p.rows){dirty();p.r++;dirty();}
result='arrow down';}else
if(c=='C'){if(p.c<p.cols){dirty();p.c++;}
result='arrow right';}else
if(c=='D'){if(p.c>1){dirty();p.c--;}
result='arrow left';}else{result=2;}}else
if(first=='('){result='define char set';p.traceEvery=true;}else
if(c=='>'){result='set numeric keypad';}else{result=2;}
...
result=null;p.esc.state=0;}}else
if((c>=' ')||c=='\n'){if(c=='<'){snew+='&lt;';p.col+=p.inc;}else
if(c=='>'){snew+='&gt;';p.col+=p.inc;}else{if(c!='\n'){if(p.isAltBuf){screen_char(c);}else{snew+=c;p.col+=p.inc;}}
if(c=='\n'||p.col==p.cols){wasScrolledDown=w3_isScrolledDown(el_scroll);if(p.isAltBuf){p.c=1;dirty();p.r++;dirty();if(p.r>p.rows)p.r=1;}else{p.tstr+=snew;if(p.tstr=='')p.tstr='&nbsp;';p.el.innerHTML=p.tstr;if(dbg)console.log('TEXT2 '+JSON.stringify(p.tstr));p.tstr=snew='';p.el=appendEmptyLine(parent_el);p.col=0;}
...
wasScrolledDown=w3_isScrolledDown(el_scroll);if(p.isAltBuf){}else{p.tstr+=snew;if(dbg)console.log('TEXT1 '+JSON.stringify(p.tstr));p.el.innerHTML=p.tstr;}
if(w3_contains(el_scroll,'w3-scroll-down')&&(!p.scroll_only_at_bottom||(p.scroll_only_at_bottom&&wasScrolledDown)))
w3_scrollDown(el_scroll);if(p.isAltBuf)sched();}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Wed Aug 31 09:46:37 2022) */
...
var kiwi_util={};var kiwi_iOS,kiwi_iPhone,kiwi_MacOS,kiwi_linux,kiwi_Windows,kiwi_android;var kiwi_safari,kiwi_firefox,kiwi_chrome,kiwi_opera,kiwi_smartTV;document.onreadystatechange=function(){if(document.readyState=="complete"){var s=navigator.userAgent;console.log(s);kiwi_iOS=(s.includes('iPhone')||s.includes('iPad'));kiwi_iPhone=s.includes('iPhone');kiwi_MacOS=s.includes('OS X');kiwi_linux=s.includes('Linux');kiwi_Windows=s.includes('Win');kiwi_android=s.includes('Android');kiwi_safari=/safari\/([0-9]+)/i.exec(s);kiwi_browserVersion=/version\/([0-9]+)/i.exec(s);kiwi_firefox=/firefox\/([0-9]+)/i.exec(s);kiwi_chrome=/chrome\/([0-9]+)/i.exec(s);kiwi_opera=/opera\/([0-9]+)/i.exec(s);if(!kiwi_opera)kiwi_opera=/OPR\/([0-9]+)/i.exec(s);kiwi_smartTV=s.includes('SmartTV');if(!kiwi_smartTV)kiwi_smartTV=s.includes('SMART-TV');if(kiwi_smartTV){if(s.includes('Samsung'))kiwi_smartTV='Samsung';else
if(s.includes('LG'))kiwi_smartTV='LG';}
console.log('MacOS='+kiwi_MacOS+' Linux='+kiwi_linux+' Windows='+kiwi_Windows+' iOS='+kiwi_iOS+' iPhone='+kiwi_iPhone+' Android='+kiwi_android+' SmartTV='+kiwi_smartTV);if(kiwi_opera){kiwi_chrome=kiwi_safari=null;}else
if(kiwi_chrome){kiwi_safari=null;}else
if(kiwi_safari){if(kiwi_browserVersion)kiwi_safari[1]=kiwi_browserVersion[1];}
console.log('Safari='+kiwi_isSafari()+' Firefox='+kiwi_isFirefox()+' Chrome='+kiwi_isChrome()+' Opera='+kiwi_isOpera());if(typeof(kiwi_check_js_version)!=='undefined'){kiwi_ajax("/VER",'kiwi_version_cb');}else{if(typeof(kiwi_bodyonload)!='undefined')
kiwi_bodyonload('');}}}
...
function kiwi_round(v,frac_digits){if(!isNumber(frac_digits)||frac_digits<0)return v;var factor=Math.pow(10,frac_digits);return Math.floor(v*factor)/ factor;}
...
return'(null)';}
function event_dump(evt,id,oneline){var k=evt.key||'(no key)';if(oneline){var trel=(isDefined(evt.relatedTarget)&&evt.relatedTarget)?(' Trel='+evt.relatedTarget.id):'';var key='';if(evt.shiftKey)key+='shift-';if(evt.ctrlKey)key+='ctrl-';if(evt.altKey)key+='alt-';if(evt.metaKey)key+='meta-';key+=k;var ct_id=evt.currentTarget?evt.currentTarget.id:'(null)';console.log('event_dump '+id+' |'+evt.type+'| k='+key+' T='+evt.target.id+' Tcur='+ct_id+trel);}else{console.log('================================');if(!isArg(evt)){console.log('EVENT_DUMP: '+id+' bad evt');kiwi_trace();return;}
...
function px(s){if(!isArg(s)||s=='')return'0';var num=parseFloat(s);if(isNaN(num)){console.log('px num='+s);kiwi_trace();return'0';}
...
function kiwi_storeGet(s,init){var rv;if(!isArg(s))return null;try{if(localStorage==null)return null;rv=localStorage.getItem(s);if(rv==null){rv=readCookie(s);if(rv!=null){console.log('$moving to storage api: '+s+'='+rv);kiwi_storeSet(s,rv);deleteCookie(s);}else{if(isDefined(init)){kiwi_storeSet(s,init);rv=init;}else{rv=null;}}}}catch(ex){console.log('$kiwi_storeGet CATCH: '+s);console.log('$kiwi_storeGet CATCH: '+ex);rv=null;}
if(rv==null)console.log('$ kiwi_storeGet warning: '+s+'=null');return rv;}
function kiwi_storeSet(s,v){if(!isArg(v))return null;try{if(localStorage==null)return null;localStorage.setItem(s,v);}catch(ex){return null;}
return true;}
function getVarFromString(path){var scope=window;var scopeSplit=path.split('.');for(i=0;i<scopeSplit.length-1;i++){var scope_name=scopeSplit[i];scope=scope[scope_name];if(isUndefined(scope)){console.log('getVarFromString: NO SCOPE '+path+' scope_name='+scope_name);throw'no scope';}}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Thu Sep  1 12:03:57 2022) */
...
if(isNaN(out_sps))out_sps=0;w3_innerHTML('id-status-audio',w3_text(optbar_prefix_color,'WF'),w3_text('',kiwi.wf_fps.toFixed(0)+' fps'),w3_text(optbar_prefix_color,'Audio'),w3_text('',(out_sps/1000).toFixed(1)+'k, Qlen '+audio_prepared_buffers.length));audio_stat_input_size=0;}
