...
/* web/extensions/ext.min.js (web/extensions/ext.js = Mon Aug 15 08:09:40 2022) */
var extint={ws:null,extname:null,srate:0,nom_srate:0,param:null,override_pb:false,displayed:false,help_displayed:false,spectrum_used:false,current_ext_name:null,using_data_container:false,hide_func:null,default_w:525,default_h:300,prev_mode:null,seq:0,scanning:0,no_lockout:['noise_blank','noise_filter','ant_switch','iframe','colormap','devl','prefs'],excl_devl:['devl','s4285','prefs'],OPT_NOLOCAL:1,};var devl={p0:0,p1:0,p2:0,p3:0,p4:0,p5:0,p6:0,p7:0};function ext_switch_to_client(ext_name,first_time,recv_func){recv_websocket(extint.ws,recv_func);ext_send('SET ext_switch_to_client='+ext_name+' first_time='+(first_time?1:0)+' rx_chan='+rx_chan);w3_call(ext_name+'_focus');}
function ext_send(msg,ws){if(ws==undefined)
ws=extint.ws;try{ws.send(msg);return 0;}catch(ex){console.log("CATCH ext_send('"+s+"') ex="+ex);kiwi_trace();}}
...
var EXT_SAVE=true;var EXT_NO_SAVE=false;function ext_get_cfg_param(path,init_val,save){var cur_val;path=w3_add_toplevel(path);try{cur_val=getVarFromString(path);}catch(ex){cur_val=null;}
if((cur_val==null||cur_val==undefined)&&init_val!=undefined){cur_val=init_val;setVarFromString(path,cur_val);if(save==undefined||save==EXT_SAVE)
cfg_save_json('ext_get_cfg_param',path);}
return cur_val;}
function ext_get_cfg_param_string(path,init_val,save){return kiwi_decodeURIComponent(ext_get_cfg_param_string+':'+path,ext_get_cfg_param(path,init_val,save));}
function ext_set_cfg_param(path,val,save){path=w3_add_toplevel(path);setVarFromString(path,val);if(save!=undefined&&save==EXT_SAVE){cfg_save_json('ext_set_cfg_param',path);}}
...
function ext_is_IQ_or_stereo_mode(mode){mode=mode.toLowerCase(mode);return(mode=='drm'||mode=='iq'||mode=='sas'||mode=='qam');}
...
function ext_set_mode(mode,freq,opt){var new_drm=(mode=='drm');if(new_drm)
...
var extint_isAdmin_cb;function ext_isAdmin(cb){extint_isAdmin_cb=cb;ext_send('SET is_admin');}
...
function ext_show_spectrum(){w3_show_block('id-spectrum-container');extint.spectrum_used=true;}
function ext_hide_spectrum(){w3_hide('id-spectrum-container');extint.spectrum_used=false;}
function extint_panel_show(controls_html,data_html,show_func,hide_func,show_help_button){extint.using_data_container=(data_html?true:false);if(extint.using_data_container){toggle_or_set_spec(toggle_e.SET,0);w3_hide('id-top-container');w3_show_block(w3_innerHTML('id-ext-data-container',data_html));}else{w3_hide('id-ext-data-container');if(!spec.source_wf)
...
function extint_panel_hide(){if(extint.using_data_container){w3_hide('id-ext-data-container');ext_hide_spectrum();w3_show_block('id-top-container');extint.using_data_container=false;if(extint.help_displayed==true){confirmation_panel_close();extint.help_displayed=false;}}
...
function extint_select_build_menu(){var s='';if(extint_names&&isArray(extint_names)){extint_names_enum(function(i,value,id,id_en){var enable=ext_get_cfg_param(id_en+'.enable');if(enable==null||kiwi.is_local[rx_chan])enable=true;if(id=='DRM')kiwi.DRM_enable=enable;if(id=='NAVTEX')id='NAVTEX/DSC';s+='<option value='+dq(value)+' kiwi_idx='+dq(i)+' '+(enable?'':'disabled')+'>'+id+'</option>';});}
return s;}
function extint_open(name,delay){name=name.toLowerCase();var found=0;extint_names_enum(function(i,value,id,id_en){var enable=ext_get_cfg_param(id_en+'.enable');if(enable==null||kiwi.is_local[rx_chan])enable=true;if(!found&&enable&&id.toLowerCase().includes(name)){if(delay){setTimeout(function(){extint_select(value);},delay);}else{extint_select(value);}
found=1;}});}
...
unknown_ext=true;if(unknown_ext){console.log('DYNLOAD UNKNOWN FILETYPE '+src);}else{script.async=false;document.head.appendChild(script);console.log('DYNLOAD loading '+src);}}else{console.log('DYNLOAD already loaded: '+src);}});console.log('DYNLOAD FINISH');if(!loaded_any){if(cb_pre){console.log('DYNLOAD call pre '+cb_pre);w3_call(cb_pre,false);}
if(cb_post){console.log('DYNLOAD call post '+cb_post);w3_call(cb_post);}}else{if(cb_pre){console.log('DYNLOAD call pre subsequent '+cb_pre);w3_call(cb_pre,true);}}}
...
var ansi={colors:[[0,0,0],[194,54,33],[37,188,36],[173,173,39],[73,46,225],[211,56,211],[51,187,200],[203,204,205],[129,131,131],[252,57,31],[49,231,34],[234,236,35],[88,51,255],[249,53,248],[20,240,240],[233,235,235]],RED:"\u001b[97m\u001b[101m",YELLOW:"\u001b[103m",GREEN:"\u001b[102m",CYAN:"\u001b[106m",BLUE:"\u001b[97m\u001b[104m",MAGENTA:"\u001b[97m\u001b[105m",GREY:"\u001b[47m",NORM:"\u001b[m",rolling:['RED','YELLOW','GREEN','CYAN','BLUE','MAGENTA','GREY'],rolling_n:7};function kiwi_output_msg(id,id_scroll,p){var dbg=(0&&dbgUs);var parent_el=w3_el(id);if(!parent_el){console.log('kiwi_output_msg NOT_FOUND id='+id);return;}
var s;try{s=kiwi_decodeURIComponent('kiwi_output_msg',p.s);}catch(ex){console.log('decodeURIComponent FAIL:');console.log(p.s);s=p.s;}
if(!p.init){p.el=w3_create_appendElement(parent_el,'pre','');p.esc={s:'',state:0};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.inc=1;p.init=true;}
var snew='';var el_scroll=w3_el(id_scroll);var wasScrolledDown=null;if(isUndefined(p.tstr))p.tstr='';if(isUndefined(p.col))p.col=0;if(p.process_return_alone&&s.charAt(0)=='\r'&&(s.length==1||s.charAt(1)!='\n')){s=s.substring(1);p.tstr=snew='';p.col=0;}
if(dbg)console.log('kiwi_output_msg:');if(dbg)console.log(JSON.stringify(p));if(dbg)console.log(JSON.stringify(s));if(p.remove_returns)s=s.replace(/\r/g,'');if(p.inline_returns){s=s.replace(/\r\n/g,'\n');s=s.replace(/\r\n/g,'\n');}
if(dbg)console.log(JSON.stringify(s));for(var i=0;i<s.length;i++){var c=s.charAt(i);if(p.inline_returns&&p.return_pending&&c!='\r'){p.tstr=snew='';p.col=0;p.return_pending=false;}
if(c=='\f'){while(parent_el.firstChild){parent_el.removeChild(parent_el.firstChild);}
p.el=w3_create_appendElement(parent_el,'pre','');p.tstr=snew='';p.col=0;}else
if(p.inline_returns&&c=='\r'){if(dbg)console.log('\\r inline:');p.return_pending=true;}else
if(c=='\t'){snew+='&nbsp;';p.col++;while((p.col&7)!=0){snew+='&nbsp;';p.col++;}}else
if(c=='\x1b'){p.esc.s='';p.esc.state=1;}else
if(p.esc.state==1){if(c<'@'||c=='['){p.esc.s+=c;}else{p.esc.s+=c;var first=p.esc.s.charAt(0);var second=p.esc.s.charAt(1);var n=0,result=0;if(first=='['){if(c=='m'){var sgr,saw_reset=0,saw_color=0;var sa=p.esc.s.substr(1).split(';');var sl=sa.length;for(var ai=0;ai<sl&&result==0;ai++){sgr=(sa[ai]==''||sa[ai]=='m')?0:parseInt(sa[ai]);if(sgr==0){p.sgr.fg=p.sgr.bg=null;saw_reset=1;}else
if(isNaN(sgr)){result=2;}else
if(sgr==1){p.sgr.bright=8;}else
if(sgr==2){p.sgr.bright=0;}else
if(sgr==22){p.sgr.bright=0;}else
if(sgr==7){var tf=p.sgr.fg;p.sgr.fg=p.sgr.bg;p.sgr.bg=tf;if(p.sgr.fg==null)p.sgr.fg=[255,255,255];if(p.sgr.bg==null)p.sgr.bg=[0,0,0];saw_color=1;}else
if(sgr==27){p.sgr.fg=p.sgr.bg=null;saw_color=1;}else
if(sgr>=30&&sgr<=37){p.sgr.fg=ansi.colors[sgr-30+p.sgr.bright];saw_color=1;}else
if(sgr>=90&&sgr<=97){p.sgr.fg=ansi.colors[sgr-90+8];saw_color=1;}else
if(sgr==39){p.sgr.fg=null;saw_color=1;}else
if(sgr>=40&&sgr<=47){p.sgr.bg=ansi.colors[sgr-40+p.sgr.bright];saw_color=1;}else
if(sgr>=100&&sgr<=107){p.sgr.bg=ansi.colors[sgr-100+8];saw_color=1;}else
if(sgr==49){p.sgr.bg=null;saw_color=1;}else
if(sgr==38||sgr==48){var n8,r,g,b,color,ci;if(sl==3&&(parseInt(sa[1])==5)&&(!isNaN(n8=parseInt(sa[2])))){ai+=2;if(n8<=15){color=ansi.colors[n8];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;saw_color=1;}else
if(n8<=231){n8-=16;r=Math.floor(n8/36);n8-=r*36;g=Math.floor(n8/6);n8-=g*6;b=n8;r=Math.floor(255*r/5);g=Math.floor(255*g/5);b=Math.floor(255*b/5);color=[r,g,b];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;saw_color=1;}else
if(n8<=255){ci=8+(n8-232)*10;color=[ci,ci,ci];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;saw_color=1;}else
result=2;}else
if(sl==5&&(parseInt(sa[1])==2)&&(!isNaN(r=parseInt(sa[2])))&&(!isNaN(g=parseInt(sa[3])))&&(!isNaN(b=parseInt(sa[4])))){r=w3_clamp(r,0,255);g=w3_clamp(g,0,255);b=w3_clamp(b,0,255);color=[r,g,b];if(sgr==38)p.sgr.fg=color;else p.sgr.bg=color;saw_color=1;}else
result=2;}else
result=2;}
if(saw_reset){if(p.sgr.span){snew+='</span>';p.sgr.span=0;}}else
if(saw_color){if(p.sgr.span)snew+='</span>';snew+='<span style="'+(p.sgr.fg?('color:'+kiwi_rgb(p.sgr.fg)+';'):'')+(p.sgr.bg?('background-color:'+kiwi_rgb(p.sgr.bg)+';'):'')+'">';p.sgr.span=1;}else{result=2;}}else
if(c=='K'){if(second.match(/[012]/))n=second-'0';result='erase in line '+["#0: cur to EOL","#1: cur to BOL","#2: full line"][n];}else
if(c=='P'){n=second-'0';result='del #'+n+' chars';}else
if(second=='?'){result='set/reset mode';}else
if(c=='r'){}else
if(c=='l'){result='reset mode';}else
if(c=='J'){if(second=='0'||second=='J')result='erase cur to EOS';else
if(second=='2'||second=='3')result='erase full screen';else
if(second=='1')result='erase BOS to cur';else
result=2;}else
if(c=='H'){result='move row,col';}else
if(c=='d'){result='move row';}else
if(c=='G'){result='move col';}else{result=2;}}else
if(first=='('){}else{result=2;}
if(result===1){if(dbg)console.log('ESC '+JSON.stringify(p.esc.s)+' IGNORED');}else
if(result===2){if(dbg)console.log('ESC '+JSON.stringify(p.esc.s)+' UNKNOWN');}else
if(isString(result)){if(dbg)console.log('ESC '+JSON.stringify(p.esc.s)+' '+result);}
p.esc.state=0;}}else
if((c>=' '&&c<='\xff')||c=='\n'){if(c=='<'){snew+='&lt;';p.col+=p.inc;}else
if(c=='>'){snew+='&gt;';p.col+=p.inc;}else{if(c!='\n'){snew+=c;p.col+=p.inc;}
if(c=='\n'||p.col==p.ncol){wasScrolledDown=w3_isScrolledDown(el_scroll);p.tstr+=snew;if(p.tstr=='')p.tstr='&nbsp;';p.el.innerHTML=p.tstr;p.tstr=snew='';p.el=w3_create_appendElement(parent_el,'pre','');p.col=0;if(w3_contains(el_scroll,'w3-scroll-down')&&(!p.scroll_only_at_bottom||(p.scroll_only_at_bottom&&wasScrolledDown)))
w3_scrollDown(el_scroll);}}}else
if(c==kiwi.esc_lt){snew+='<';p.inc=0;}else
if(c==kiwi.esc_gt){snew+='>';p.inc=1;}}
wasScrolledDown=w3_isScrolledDown(el_scroll);p.tstr+=snew;p.el.innerHTML=p.tstr;if(w3_contains(el_scroll,'w3-scroll-down')&&(!p.scroll_only_at_bottom||(p.scroll_only_at_bottom&&wasScrolledDown)))
...
function w3_call(func,arg0,arg1,arg2,arg3,arg4){var rv=undefined;if(!isArg(func))return rv;try{if(isString(func)){var f=getVarFromString(func);if(isFunction(f)){rv=f(arg0,arg1,arg2,arg3,arg4);}else{}}else
...
function w3_disable(el,disable){w3_set_props(el,'w3-disabled',disable);}
...
function w3_colors(el_id,colors){var el=w3_el(el_id);if(!el)return null;var ar=colors?colors.split(','):null;var bg=null,fg=null;if(ar&&ar.length>0)bg=ar[0];if(ar&&ar.length>1)fg=ar[1];if(bg&&bg.startsWith('w3-'))
w3_add(el,bg);else
el.style.backgroundColor=bg;if(fg&&fg.startsWith('w3-'))
w3_add(el,fg);else
el.style.color=fg;}
...
return path;}
function w3_psa3(psa3){psa3=psa3||'';a=psa3.split('/');var a0=(a[0]||'').replace('&slash;','/');var a1=(a[1]||'').replace('&slash;','/');var a2=(a[2]||'').replace('&slash;','/');if(a.length==1)
...
function w3_sb(){var a=[];for(var i=0;i<arguments.length;i++){var s=arguments[i];s=isString(s)?s.trim():'';if(s!='')a.push(s);}
return a.length?a.join(' '):'';}
function w3_sbc(c){var a=[];for(var i=1;i<arguments.length;i++){var s=arguments[i];s=isString(s)?s.trim():'';if(s!='')a.push(s);}
return a.length?a.join(c):'';}
function w3_sbew(ew){var a=[];for(var i=1;i<arguments.length;i++){var s=arguments[i];s=isString(s)?s.trim():'';if(s!=''){if(s[s.length-1]!=ew)s=s+ew;a.push(s);}}
return a.length?a.join(' '):'';}
function w3_psa(psa,extra_prop,extra_style,extra_attr){if(psa&&psa.startsWith('class=')){return psa;}
var a=psa?psa.split('|'):[];psa='';var prop=a[0]||'';if(extra_prop)prop=w3_sb(prop,extra_prop);if(prop!='')psa='class='+dq(prop);var style=a[1]||'';if(extra_style)style=w3_sbew(';',style,extra_style);if(style!='')psa=w3_sb(psa,'style='+dq(style));var attr=(a[2]||'').replace('&vbar;','|');if(extra_attr)attr=w3_sb(attr,extra_attr);if(attr!='')psa=w3_sb(psa,attr);return psa;}
...
function w3_do_when_rendered(id,func,arg,poll_ms){var el=w3_el(id);poll_ms=isNumber(poll_ms)?poll_ms:500;if(!isArg(el)){setTimeout(function(){w3_do_when_rendered(id,func,arg,poll_ms);},poll_ms);return;}
func(el,arg);}
function w3_do_when_cond(cond_func,func,arg,poll_ms){poll_ms=isNumber(poll_ms)?poll_ms:500;if(!cond_func()){setTimeout(function(){w3_do_when_cond(cond_func,func,arg,poll_ms);},poll_ms);return;}
func(arg);}
...
function w3int_link_click(ev,cb,cb_param){console.log('w3int_link_click cb='+cb+' cb_param='+cb_param);console.log(ev);var el=ev.currentTarget;console.log(el);w3_check_restart_reboot(el);if(cb){w3_call(cb,el,cb_param,false);}
w3int_post_action();}
function w3_link(psa,url,inner,title,cb,cb_param){var qual_url=url;var target;if(url.startsWith('javascript:')){target='';}else{if(url!=''&&!url.startsWith('http://')&&!url.startsWith('https://'))
qual_url='http://'+url;target=' target="_blank"';}
inner=inner||'';title=title||'';if(title!='')title=' title='+dq(title);var pointer=(cb&&cb!='')?'w3-pointer':'';cb_param=cb_param||0;var onclick=cb?(' onclick="w3int_link_click(event, '+sq(cb)+', '+sq(cb_param)+')"'):'';var p=w3_psa(psa,pointer,'','href='+dq(qual_url)+target+title+onclick);var esc_html=psa.includes('w3-esc-html');var lt=esc_html?kiwi.esc_lt:'<';var gt=esc_html?kiwi.esc_gt:'>';var s=lt+'a '+p+gt+inner+lt+'/a'+gt;return s;}
function w3_link_set(path,url){var el=w3_el(path);if(el&&url!=''){if(!url.startsWith('http://')&&!url.startsWith('https://'))
url='http://'+url;w3_attribute(el,'href',url);}}
...
function w3int_button_click(ev,path,cb,cb_param){if(!w3_contains(path,'w3-disabled')){w3_check_restart_reboot(ev.currentTarget);if(cb){w3_call(cb,path,cb_param,false);}}
w3int_post_action();}
var w3int_btn_grp_uniq=0;function w3_btn(text,cb){console.log('### DEPRECATED: w3_btn');return w3_button('',text,cb);}
function w3_cb_param_encode(cbp){if(!isArg(cbp)){return 0;}
if(isObject(cbp)){return encodeURIComponent(JSON.stringify(cbp));}
return cbp;}
function w3_cb_param_decode(cbp){if(!isArg(cbp)){return 0;}
if(isString(cbp)){cbp=decodeURIComponent(cbp);var obj;if(cbp[0]=='{'){try{obj=JSON.parse(cbp);}catch(ex){return cbp;}
return obj;}}
return cbp;}
function w3int_button(psa,path,text,cb,cb_param){cb_param=w3_cb_param_encode(cb_param);var onclick=cb?('onclick="w3int_button_click(event, '+sq(path)+', '+sq(cb)+', '+sq(cb_param)+')"'):'';if(cb&&psa.includes('w3-momentary')){onclick+=' onmousedown="w3int_button_click(event, '+sq(path)+', '+sq(cb)+', 0)"';onclick+=' ontouchstart="w3int_button_click(event, '+sq(path)+', '+sq(cb)+', 0)"';}
var default_style=(psa.includes('w3-round')||psa.includes('w3-circle'))?'':' w3-round-large';var psa3=w3_psa3(psa);var psa_outer=w3_psa(psa3.left);var psa_inner=w3_psa(psa3.right,path+' w3-btn w3-ext-btn'+default_style,'',onclick);if(psa.includes('w3-dump')){console.log('w3_button');console.log(psa3);console.log('psa_outer='+psa_outer);console.log('psa_inner='+psa_inner);}
...
function w3_button_text(path,text,color_or_add_color,remove_color){var el=w3_el(path);if(!el)return null;el.innerHTML=text;if(color_or_add_color){if(color_or_add_color.startsWith('w3-')){w3_remove(el,remove_color);w3_add(el,color_or_add_color);}else
el.style.color=color_or_add_color;}
return el;}
function w3_icon(psa,fa_icon,size,color,cb,cb_param){var pointer=(cb&&cb!='')?' w3-pointer':'';var path='id-btn-grp-'+w3int_btn_grp_uniq.toString();w3int_btn_grp_uniq++;cb_param=cb_param||0;var font_size=null;if(isNumber(size)&&size>=0)font_size=px(size);else
if(isString(size))font_size=size;font_size=font_size?(' font-size:'+font_size+';'):'';color=color||'';var c=color.split('|');color='';if(c[0]!='')color=' color:'+c[0]+';';if(c.length>=2&&c[1]!='')color+=' background-color:'+c[1]+';';var onclick=cb?('onclick="w3int_button_click(event, '+sq(path)+', '+sq(cb)+', '+sq(cb_param)+')"'):'';if(cb&&psa.includes('w3-momentary')){onclick+=' onmousedown="w3int_button_click(event, '+sq(path)+', '+sq(cb)+', 0)"';onclick+=' ontouchstart="w3int_button_click(event, '+sq(path)+', '+sq(cb)+', 0)"';}
var p=w3_psa(psa,path+pointer+' fa '+fa_icon,font_size+color,onclick);var s='<i '+p+'></i>';return s;}
function w3int_input_keydown(ev,path,cb){var i,s;var k=ev.key.toUpperCase();var ctl=ev.ctrlKey;var el=w3_el(path);if(!el)return;var trace=w3_contains(el,'w3-trace');if(trace)console.log('w3int_input_keydown k='+k+(ctl?' CTL ':'')+' val=<'+el.value+'> cb='+cb);var cb_a=cb.split('|');if(ctl&&'CD\\'.includes(k)&&cb_a[1]){w3_call(cb_a[1],k);}
var input_any_change=w3_contains(el,'w3-input-any-change');if(ev.key=='Enter'){if(input_any_change){s=el.value;for(i=el.selectionStart;i<el.textLength;i++){if(s.charAt(i)!='\n')
break;}
...
function w3_textarea(psa,label,path,val,rows,cols,cb){var id=w3_add_id(path);var spacing=(label!='')?' w3-margin-T-8':'';var onchange=psa.includes('w3-input-any-change')?'':(' onchange="w3_input_change('+sq(path)+', '+sq(cb)+')"');var onkeydown=' onkeydown="w3int_input_keydown(event, '+sq(path)+', '+sq(cb)+')"';var onkeyup=' onkeyup="w3int_input_keyup(event, '+sq(path)+', '+sq(cb)+')"';var val=val||'';var bold=!psa.includes('w3-label-not-bold');var psa3=w3_psa3(psa);var psa_label=w3_psa_mix(psa3.middle,(label!=''&&bold)?'w3-bold':'');var psa_inner=w3_psa(psa3.right,'w3-input w3-border w3-hover-shadow '+id+spacing,'','rows='+dq(rows)+' cols='+dq(cols));var s=w3_div(psa3.left,w3_label(psa_label,label,path)+'<textarea '+psa_inner+onchange+onkeydown+onkeyup+'>'+val+'</textarea>');return s;}
...
function w3_checkbox_get_param(psa,label,path,cb,init_val){var cur_val=ext_get_cfg_param(path,(init_val==undefined)?null:init_val);return w3_checkbox(psa,label,path,cur_val,cb);}
...
s+='<option value='+dq(idx++)+' '+(el[1]?'':'disabled')+'>'+el[0]+'</option> ';});return w3int_select(psa,label,title,path,sel,s,cb,cb_param);}
...
function w3_select_get_param(psa,label,title,path,opts,cb,init_val){var cur_val=ext_get_cfg_param(path,(init_val==undefined)?0:init_val);return w3_select(psa,label,title,path,cur_val,opts,cb);}
...
setTimeout(function(){w3_call(cb,path,val,true,true,cb_param);},500);return s;}
...
function w3_num_set_cfg_cb(path,val,first){var v=parseFloat(val);if(isNaN(v))v=0;var save=(first!=undefined)?(first?false:true):true;ext_set_cfg_param(path,v,save);}
function w3_int_set_cfg_cb(path,val,first){var v=parseInt(val);if(isNaN(v))v=0;w3_set_value(path,v);var save=(first!=undefined)?(first?false:true):true;ext_set_cfg_param(path,v,save);}
...
v=false;var save=(first!=undefined)?(first?false:true):true;ext_set_cfg_param(path,v,save);}
function w3_string_set_cfg_cb(path,val,first){var save=(first!=undefined)?(first?false:true):true;ext_set_cfg_param(path,encodeURIComponent(val.toString()),save);}
function w3_json_set_cfg_cb(path,val,first){var save=(first!=undefined)?(first?false:true):true;ext_set_cfg_param(path,val,save);}
...
function w3_code(prop_outer,prop_inner){var narg=arguments.length;var s='<pre class="'+prop_outer+'"><code>';for(var i=2;i<narg;i++){s+='<div class="'+prop_inner+'">'+arguments[i]+'</div>';}
s+='</code></pre>';return s;}
